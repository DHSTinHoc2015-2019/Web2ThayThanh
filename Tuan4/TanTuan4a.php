...
<body style="font-size: 20px; font-weight: bold; margin-top: 3em; margin-left: 10em; margin-right: 10em;">
	<?php 
		function SoLonThuHai($mang)
		{
			...
			//Tìm phàn tử nhỏ nhất của mảng để gán cho max1 và max2
			$min = $mang[0];
			for ($i=0; $i < count($mang); $i++) { 
				if($min > $mang[$i]) $min = $mang[$i];
			}
			//Đặt số lớn nhất là max1 và số lớn thứ 2 là max2
			$max1 = $max2 = $min;
			...
			for ($i=0; $i < count($mang); $i++) {
				...
				else if($max2 < $mang[$i] && $mang[$i] < $max1) $max2 = $mang[$i];
			}
